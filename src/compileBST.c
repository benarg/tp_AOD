...
int main (int argc, char *argv[]) {
  ...
  initialisation(n, freqFile); // initialisation des tableaux
  bellman(0, n - 1); // on remplit le tableaux des sommets de chaques sous arbres optimals ainsi que le tableau qui permet d'acceder aux nombres de comparaisons moyen pour acceder a un element de l'arbre
  bstree(0, n - 1); // on construit l'arbre ideal (tableau de l'arbre ideal)
  afficher_bst(n); // on affiche cet arbre
